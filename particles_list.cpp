...
void particles_list::step_v(E_field *e_fld, H_field *h_fld, Time* t)
{
	int i=0;
    #ifdef BUILD_CUDA
	  TransferEHToCUDA (e_fld->get_1d_e1(), e_fld->get_1d_e2(), e_fld->get_1d_e3(), h_fld->get_1d_h1(), h_fld->get_1d_h2(), h_fld->get_1d_h3());
    #endif
	for(i=0;i<part_list.size();i++)
	{
	  #ifdef BUILD_CUDA
        ...
        TransferXVToCUDA(part_list[i]->x1, part_list[i]->x3, part_list[i]->v1, part_list[i]->v2, part_list[i]->v3);
	    CUDA_StepV(t->delta_t);
	    TransferXVFromCUDA(part_list[i]->x1, part_list[i]->x3, part_list[i]->v1, part_list[i]->v2, part_list[i]->v3);
      #else
